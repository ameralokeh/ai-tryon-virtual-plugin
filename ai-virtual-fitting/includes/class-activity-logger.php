...
class AI_Virtual_Fitting_Activity_Logger {
    ...
    public function log_activity($user_id, $action, $product_id = 0, $product_name = '', $status = 'success', $error_message = '', $processing_time = 0) {
        ...
        // Define format for each field - must match log_data array exactly
        $format = array(
            '%d', // user_id
            '%s', // user_email
            '%s', // user_name
            '%s', // action
            '%d', // product_id (can be NULL)
            '%s', // product_name (can be NULL)
            '%s', // status
            '%s', // error_message (can be NULL)
            '%s', // ip_address
            '%s', // user_agent
            '%f', // processing_time (can be NULL)
            '%s'  // created_at
        );
